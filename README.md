...
## Was die App bietet
- **Geführter Flow**: Import & Mapping → Validation → Abweichungen & Repräsentativität (LLM-gestützt, Kommentar- + Parameter-Spalten einmal wählen) → Reaktionsnetzwerk → Modeling/Fit (Varianten rechnen) → Report/Export (LLM-unterstützt).
...
Das App-Layout folgt dem UI-Design-Draft unter `design/kinetik-researcher.design-draft.html` (Design-Vertrag, kein Produktionscode). Öffne die Datei im Browser, um das neue End-to-End-UI zu sehen. Implementierungen in `app/` sollen die dortige Informationsarchitektur und Kerninteraktionen funktional widerspiegeln:
 ...
 - Horizontaler Stepper mit Fortschrittsbalken (6 Schritte, Abweichungen + Repräsentativität kombiniert)
...
- **Modeling**: Modellannahmen ergänzen (z.B. Katalysator-Deaktivierung, unbekannte Nebenpfade), Modeling starten und alle Varianten mit Gleichungen, Kennzahlen und Diagrammen vergleichen.
