...
## Was die App bietet
- **Geführter Flow**: Import & Mapping → Validation → Abweichungen & Repräsentativitäts-Check (LLM-unterstützt, Kommentar- + Parameterspalten) → Modeling/Fit → Report/Export (LLM-unterstützt).
...
- **LLM klar ausgewiesen**: Abweichungen und der Repräsentativitäts-Check laufen in einem kombinierten Schritt bevorzugt mit GPT-5 mini (Snapshot 2025-08-07); pro Experiment zwei Aufrufe nacheinander, Fallback auf gpt-5-mini.
...
- Horizontaler Stepper mit Fortschrittsbalken (5 Schritte, Abweichung & Repräsentativität kombiniert)
...
- **Abweichungen & Repräsentativitäts-Check (LLM)**: Kombinierter Schritt statt Grouping. Nutzer:innen wählen einmal Kommentar- und Parameterspalten, das LLM liest die Einträge pro Experiment (Call 1), markiert Abweichungen und führt direkt danach den Referenzabgleich (Call 2) inkl. Fit-Empfehlung durch. Ergebnis: Abweichungs-Liste inkl. Herkunftsspalte, Repräsentativitäts-Findings und Fit-Auswahl je Experiment in einer gemeinsamen Kachel.
...
- Nutzer:innen wählen einmal Kommentar- und Parameterspalten; das LLM markiert Auffälligkeiten pro Experiment (Call 1) und führt direkt danach den Repräsentativitäts-Check aus (Call 2). Die Analyse erfolgt bevorzugt via GPT-5 mini (Snapshot 2025-08-07), mit Fallback auf gpt-5-mini.
- Der Repräsentativitäts-Check liefert eine Fit-Empfehlung samt Kurzsummary. Experimente werden nicht automatisch ausgeschlossen, sondern können gezielt für das Fitting vorgemerkt werden.
- Der bisherige Grouping-Schritt entfällt; UI und Navigation müssen den kombinierten Schritt sichtbar machen (Stepper auf 5 Schritte).

### LLM-Abweichungsanalyse (Schritt 3, Call 1)
- Auswahl: Kommentarspalten (potenzielle Abweichungen) und Parameterspalten (z.B. Temperatur, Edukte, Additive, Lösemittel).
...
### LLM-Repräsentativitäts-Check (Schritt 3, Call 2)
- Auswahl: Referenzspalten (z.B. Edukt, Temperatur, Konzentration, Charge) und Kontext-/Kommentarspalten (wie oben gewählt).
