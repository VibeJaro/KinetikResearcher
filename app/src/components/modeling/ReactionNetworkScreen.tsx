...
type ReactionNetworkScreenProps = {
  ...
  onNetworkChange: (nextState: ReactionNetworkState) => void;
  onBack: () => void;
  onConfirm: () => void;
};
...
export const ReactionNetworkScreen = ({
  ...
  const { assignments, edges } = networkState;
  ...
  useEffect(() => {
    if (selectedSeries.length === 0) {
      if (assignments.length > 0 || edges.length > 0) {
        onNetworkChange({ assignments: [], edges: [], confirmed: false });
      }
      return;
    }

    const prevByColumn = new Map(assignments.map((item) => [item.column, item]));
    const nextAssignments = selectedSeries.map((series, index) => {
    ...
    const hasChanges =
      ...
      nextAssignments.some((item, index) => {
    ...
    if (hasChanges) {
      onNetworkChange({ assignments: nextAssignments, edges, confirmed: false });
    }
  }, [assignments, edges, onNetworkChange, selectedSeries]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      nextEdges.some((edge, index) => edges[index]?.id !== edge.id)
    ) {
      onNetworkChange({ assignments, edges: nextEdges, confirmed: false });
    }
  }, [assignments, edges, onNetworkChange]);
  ...
  const handleEdgeChange = (id: string, key: "source" | "target" | "type", value: string) => {
    onNetworkChange({
      assignments,
      edges: edges.map((edge) => (edge.id === id ? { ...edge, [key]: value } : edge)),
      confirmed: false
    });
  };
  ...
  const handleAddEdge = () => {
    ...
    onNetworkChange({
      assignments,
      edges: [...edges, createEdge(first, second, edges.length + 1)],
      confirmed: false
    });
  };

  return (
    <div className="modeling-screen" aria-labelledby="network-heading">
      <div className="section-intro">
        <p className="eyebrow">Schritt 4 · Reaktionsnetzwerk</p>
        <h2 id="network-heading">Reaktionsnetzwerk definieren</h2>
        <p className="muted">
          Bevor das Modeling startet, ordnest du die gemessenen Spalten den Rollen zu und
          definierst den Pfad zwischen den Komponenten. So ist eindeutig, wo Nebenprodukte oder
          Nebenpfade entstehen können.
        </p>
      </div>
          ...
          <div className="card-body">
            ...
            <div className="hint-card">
              ...
              <ul>
                ...
                <li>
                  Zwischenprodukte sollten mindestens eine eingehende und eine ausgehende Kante
                  haben.
                </li>
