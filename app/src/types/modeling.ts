...
export type ModelingVariantScore = {
  r2: number;
  rmse: number;
  aic: number;
  bic: number;
...
export type ModelingChartSeries = {
  time: number[];
  observed: number[];
  predicted: number[];
};
...
export type ModelingVariant = {
  ...
  assumptions: string[];
  equations: string[];
  weighting: "gleich" | "zeitgewichtet";
  score: ModelingVariantScore;
  chart: ModelingChartSeries;
};
